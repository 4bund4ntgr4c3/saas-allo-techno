    ...
    <div ref={formTopRef} className="border border-border bg-card p-5 sm:p-7 rounded-2xl max-w-2xl mx-auto space-y-6 shadow-xl animate-in fade-in duration-200">
      ...
      {receipt ? (
        <div className="border border-emerald-600/30 bg-surface/80 p-6 rounded-2xl text-center space-y-4 shadow-sm animate-in zoom-in-95 duration-200 text-xs">
          ...
          <h4 className="text-lg font-bold text-foreground">Sauvegarde Cloud Sécurisée &amp; Verrouillée</h4>
          ...
          <div className="p-4 rounded-xl bg-background border border-border space-y-2 max-w-md mx-auto text-left">
            <div className="flex justify-between items-center text-xs">
              ...
              <strong className="font-mono text-foreground">{receipt.totalSizeMegabytes / 1000} Go Chiffrés</strong>
            </div>
            <div className="flex justify-between items-center text-xs">
              <span className="text-muted-foreground">Code PIN de Restauration :</span>
              <strong className="font-mono text-base font-extrabold text-emerald-600">{receipt.retrievalPinCode}</strong>
            </div>
          ...
          <p className="text-[11px] text-muted-foreground max-w-md mx-auto">
            Vos données sont conservées pendant {receipt.expirationDays} jours dans notre datacenter souverain. Conservez votre code PIN précieusement.
          </p>
        ...
        <form onSubmit={handleCreateBackup} className="space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-muted-foreground block mb-1">Nom &amp; Prénom du Propriétaire :</label>
              <Input
                required
            ...
            <div>
              <label className="text-muted-foreground block mb-1">Téléphone Mobile (Réception PIN) :</label>
              <Input
                required
                value={clientPhone}
          ...
          <div className="space-y-2">
            <label className="text-muted-foreground block">Dossiers vitaux à chiffrer et sauvegarder :</label>
            <div className="space-y-2">
              {[
                "Dossier Bureau & Documents",
                "Comptabilité & Factures",
          ...
          >
            {loading ? <Loader2 className="size-4 mr-1.5 animate-spin" /> : <Lock className="size-4 mr-1.5" />}
            {loading ? "Chiffrement AES-256 en cours..." : "Sécuriser mes Fichiers dans le Cloud Allô Techno"}
          </Button>
        </form>
      )}
    </div>
