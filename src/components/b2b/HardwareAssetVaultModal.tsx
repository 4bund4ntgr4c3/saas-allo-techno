...
export function HardwareAssetVaultModal() {
  ...
  const [incidentDetails, setIncidentDetails] = React.useState(
    "Vol avec effraction dans véhicule de fonction à Ganhi Cotonou",
  );
  const [loading, setLoading] = React.useState(false);
  const [stolenReportSuccess, setStolenReportSuccess] = React.useState<{
    pv: string;
    msg: string;
  } | null>(null);
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Protection cryptographique des numéros de série, UUID cartes mères et blocage réseau
              immédiat
            </p>
          </div>
        </div>
      ...
      {stolenReportSuccess ? (
        <div className="border border-destructive/40 bg-destructive/5 p-6 rounded-2xl text-center space-y-4 shadow-sm animate-in zoom-in-95 duration-200">
          ...
          <h4 className="text-lg font-bold text-destructive">
            Signalement de Vol &amp; Blacklistage Activé
          </h4>
          <Badge
            variant="outline"
            className="font-mono text-sm text-destructive border-destructive/40 font-bold"
          >
            PV Déclaration : {stolenReportSuccess.pv}
          </Badge>
          <p className="text-xs text-muted-foreground max-w-md mx-auto leading-relaxed">
            {stolenReportSuccess.msg} Le numéro de série et l'UUID de carte mère sont verrouillés
            dans toute la zone UEMOA.
          </p>
          <Button
            type="button"
            variant="outline"
        ...
        <div className="space-y-6 text-xs">
              ...
              {assets.map((a) => (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-border/60 pb-2">
                    ...
                    <Badge
                      variant="outline"
                      className="text-emerald-600 border-emerald-600/30 text-[10px] font-bold"
                    >
          ...
          <form
            onSubmit={handleReportStolen}
            className="p-4 rounded-xl bg-destructive/5 border border-destructive/30 space-y-3"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-muted-foreground block mb-1 text-[11px]">
                  Actif concerné :
                </label>
                <select
            ...
            >
              ...
              {loading
                ? "Verrouillage Réseau..."
                : "Blacklister l'Actif & Générer le PV de Vol Police"}
            </Button>
          </form>
        </div>
      )}
