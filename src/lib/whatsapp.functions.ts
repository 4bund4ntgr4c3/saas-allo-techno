...
export const sendWhatsAppTicketNotificationFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ success: boolean; whatsappUrl: string; messageText: string }> => {
    let statusText = "pris en charge par notre équipe d'experts à l'atelier Allô Techno.";
    if (data.status === "diagnostic_ready") {
      statusText = "diagnostiqué. Le rapport technique et le devis sont disponibles sur votre espace B2B.";
    } else if (data.status === "repaired") {
      statusText = "entièrement réparé et testé sur notre banc d'essai. Il est prêt à être récupéré !";
    } else if (data.status === "delivered") {
      statusText = "livré avec succès sur votre site.";
    }

    const messageText = `👋 Bonjour ${data.recipientName},\n\n` +
