  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: "Passez le test technique d'évaluation en micro-soudure et électronique pour rejoindre l'équipe Allô Techno.",
        },
        ...seo.meta,
      ],
      links: [...seo.links],
...
const QUESTIONS: Question[] = [
  {
    id: 1,
    question: "Sur une carte mère MacBook, une ligne PPBUS_G3H mesurée à 0V avec une impédance de 0.2 Ohm à la masse indique généralement :",
    options: [
    ...
    correctIndex: 1,
    explanation: "Une impédance quasi-nulle (0.2 Ohm) caractérise un court-circuit franc à la masse (souvent un condensateur céramique ou un transistor MOSFET en claquage).",
  },
  {
    id: 2,
    question: "Quelle température de buse d'air chaud et quel flux d'air sont généralement préconisés pour dessouder un connecteur plastique FPC sans le faire fondre ?",
    options: [
    ...
    correctIndex: 1,
    explanation: "L'abaissement du point de fusion avec un alliage étain/bismuth (138°C) et un chauffage doux par le dessous protègent le plastique fragile du connecteur.",
  },
  {
    id: 3,
    question: "Lors d'une désoxydation d'une carte mère tombée dans l'eau, pourquoi le nettoyage aux ultrasons doit-il être suivi d'un séchage sous étuve ?",
    options: [
    ...
    correctIndex: 1,
    explanation: "L'humidité résiduelle sous les billes d'étain BGA provoque des micro-courts-circuits instantanés dès la première impulsion électrique.",
  },
  {
    id: 4,
    question: "Sur un PC portable Dell ou HP qui démarre avec écran noir et clignotement LED 3 blancs / 2 ambrés, le code d'erreur indique :",
    options: [
    ...
    correctIndex: 0,
    explanation: "Le code 3-2 sur Dell/HP correspond au standard diagnostic de non-détection ou corruption de barrette mémoire RAM.",
  },
  {
    id: 5,
    question: "Quelle précaution absolue est indispensable AVANT de déconnecter la nappe d'écran d'un MacBook ou PC moderne ?",
    options: [
    ...
    correctIndex: 1,
    explanation: "Le rail d'alimentation du rétroéclairage LED (Backlight 35V-50V) reste sous tension permanente si la batterie est branchée : débrancher la nappe sous tension détruit instantanément le circuit driver rétroéclairage.",
  },
];
      ...
      <section className="border-b border-border py-12 bg-surface/40">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <PageBreadcrumb items={[{ label: "Rejoindre l'équipe" }, { label: "Test Technique" }]} />
          </div>
          <h1 className="at-display text-3xl sm:text-4xl font-extrabold text-foreground">
            Évaluation Technique &amp; Micro-Soudure
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Testez vos connaissances en diagnostic de composants, électronique de puissance et architecture informatique pour postuler chez Allô Techno.
          </p>
        </div>
      </section>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {submitted && (
                  <Badge variant="outline" className={isCorrect ? "text-emerald-600 border-emerald-600/40" : "text-destructive border-destructive/40"}>
              ...
              <div className="space-y-2">
                {q.options.map((opt, optIdx) => {
                  ...
                  let btnStyle = "border-border bg-surface text-muted-foreground hover:text-foreground";
                  if (submitted) {
                    if (isAnswerCorrect) btnStyle = "border-emerald-600 bg-emerald-600/20 text-emerald-800 font-bold";
                    else if (isSelected) btnStyle = "border-destructive bg-destructive/20 text-destructive font-bold";
                  } else if (isSelected) {
          ...
          {!submitted ? (
            <>
              <div className="text-xs text-muted-foreground">
                Questions répondues : <strong>{Object.keys(selectedAnswers).length} / {QUESTIONS.length}</strong>
              </div>
              <Button
                variant="technical"
                disabled={!isComplete}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Score : <strong>{Math.round((score / QUESTIONS.length) * 100)}% de bonnes réponses</strong>
                  </p>
                </div>
              </div>
