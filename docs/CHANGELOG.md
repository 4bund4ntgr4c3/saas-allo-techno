...
- **Architecture / Admin** : migration de 15 onglets admin du client Supabase vers des server functions (PII et donnees lues cote serveur uniquement) — AdminDashboard/StatsDashboard/AdminAnalytics/AdminTeam/AdminOrders/AdminLeadsClaims/AdminChecklistModal/AdminContent/AdminStock/AdminDeliveries/AdminSatisfaction/AdminPOS/AdminKanban/AdminChat ; nouvelles server fns dans admin.functions.ts (getAdminDashboardStats, getAdminAnalyticsData, getAdminTeamData, getAdminStatsData, getAdminOrdersData, getAdminLeadsData, getAdminDeliveries, getAdminCompletedDossiers, searchAdminReservations, getReservationStatusHistory, getAdminConversationReservations, saveChecklist, setLeadStatus, setTeamRole, requireStaffGuard) et content.functions.ts (getAdminBlogPosts, getAdminBlogPost, getAdminReviews, getAdminLowStock, getAdminStock), toutes gardees par is_staff + rateLimit cote serveur ; seuls restent cote client les canaux Realtime (chat, atelier) et logAudit (écriture staff via RLS).
