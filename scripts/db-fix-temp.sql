-- TEMP db-fix: migrations manquantes en prod
-- 1) Batch 38 B2B (20260818000000) jamais appliquee + 2) reservations.description (20260825000000)
...
drop policy if exists "Org members can view invoices" on public.organization_invoices;
drop policy if exists "Org admins can manage invoices" on public.organization_invoices;
...
drop policy if exists "Org members can view invoice items" on public.organization_invoice_items;
...
drop policy if exists "Org members can view maintenance schedules" on public.equipment_maintenance_schedules;
drop policy if exists "Org admins can manage maintenance schedules" on public.equipment_maintenance_schedules;
...
-- 3. reservations.description (B2B tickets query)
